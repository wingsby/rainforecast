...
from labs.BasicTrajGRUModel import forward
...
sequence_length = 13
index = int(np.floor(sequence_length / 2)) + 1
    ...
    with tf.device('/cpu:0'):
        ...
        for i in range(num_gpus):
            ...
            with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                ...
                psnr_all = psnr_all
                loss = loss = loss / np.float32(len(gen_images))*100
                # loss = loss
        ...
        return output, train_op, lr, loss, summ_op
...
def main():
    ...
    with tf.variable_scope('model', reuse=None) as training_scope:
        ...
        output, train_op, lr, loss, summ_op = train(images, sequence_length,
                                                    prefix='train')
