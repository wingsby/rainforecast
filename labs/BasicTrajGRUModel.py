...
def forward(images, index, reuse=None):
    ...
    for i in range(images.__len__()):
        ...
        with slim.arg_scope(
            ...
            # enc0 = slim.layers.conv2d(prev_image, lstm_size[2], 5, stride=2, scope='preinp',
            #                           normalizer_fn=tf_layers.layer_norm,
            #                           normalizer_params={'scope': 'layer_norm1'})
            hidden1, lstm_state1= gru_fun(prev_image, lstm_state1,  lstm_size[0], filter_size=7,
                                                        scope="sate1")
            hidden1 = tf_layers.layer_norm(hidden1, scope='layer_norm2')
            hidden2, lstm_state2 = gru_fun(hidden1, lstm_state1, lstm_size[1], filter_size=5,
                                                        scope="sate2")
            hidden2 = tf_layers.layer_norm(hidden2, scope='layer_norm3')
            # hidden3, lstm_state2 = gru_fun(hidden2, lstm_state2,  lstm_size[1], filter_size=3,
            #                                             scope="sate3")
            # hidden3 = tf_layers.layer_norm(hidden3, scope='layer_norm4')
            # hidden4, lstm_state4 = gru_fun(hidden3, lstm_state3,  lstm_size[1], filter_size=3,
            #                                             scope="sate4")
            # hidden4 = tf_layers.layer_norm(hidden4, scope='layer_norm5')
            # enc1 = slim.layers.conv2d_transpose(
            #     hidden2, hidden2.get_shape()[3], 3, stride=2, scope='convt1')
            output = slim.layers.conv2d(hidden2, 1, 1, stride=1, scope='finalout')
